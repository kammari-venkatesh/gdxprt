...
const links = {
  Company: ['About Us', 'Careers', 'Blog', 'Privacy Policy', 'Terms & Conditions'],
  Support: ['FAQ', 'Contact Us', 'Help Center'],
  Resources: ['Counselor Login', 'Student Login', 'Testimonials'],
}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20">
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-10">
          <div className="lg:col-span-2">
            <a href="#" className="inline-block"><Logo light /></a>
            <p className="mt-5 text-gray-400 text-sm max-w-sm leading-relaxed">
              GuideXpert empowers career counselors and students with training, tools, and a supportive community. Shape futures and unlock opportunities.
            </p>
            <div className="flex gap-3 mt-6">
              {social.map((s) => (
                <a
                  key={s.name}
                  href={s.href}
                  className="w-10 h-10 rounded-xl bg-white/5 hover:bg-primary flex items-center justify-center text-gray-400 hover:text-white transition-all duration-300"
                  aria-label={s.name}
                >
          ...
          {Object.entries(links).map(([heading, items]) => (
            <div key={heading}>
              <h3 className="font-display font-semibold text-white mb-4">{heading}</h3>
        ...
        <div className="mt-14 pt-8 border-t border-white/10 text-center">
          <p className="text-gray-500 text-sm">© 2026 GuideXpert. All rights reserved.</p>
        </div>
      </div>
    </footer>
  )
